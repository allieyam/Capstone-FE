...
function loadFeatures(filePath, numWords, maxLen, multihot = false) {
  ...
  if (multihot) {
    ...
    const paddedSequences = padSequences(sequences, maxLen, "pre", "pre");
    return tf.tensor2d(
      paddedSequences,
      [paddedSequences.length, maxLen],
      "int32"
    );
  }
}
...
async function maybeDownloadAndExtract() {
  ...
  const zipExtractDir = zipDownloadDest.slice(
    0,
    zipDownloadDest.length - ZIP_SUFFIX.length
  );

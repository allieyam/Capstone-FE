...
//     trial = model.predict(tf.tensor2d([5], [1, 1]));
...
function parseArguments() {
  ...
  parser.addArgument("--maxLen", {
    ...
    defaultValue: 100,
    help:
      "Maximum sentence length in number of words. " +
  ...
  parser.addArgument("--embeddingFilesPrefix", {
    ...
    defaultValue: "",
    help:
      "Optional path prefix for saving embedding files that " +
...
async function main() {
  ...
  const [testLoss, testAcc] = model.evaluate(xTest, yTest, {
    batchSize: args.batchSize,
  });
  ...
  if (
    args.embeddingFilesPrefix != null &&
